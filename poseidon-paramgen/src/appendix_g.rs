...
mod tests {
    ...
    fn poseidon_bls12_377_instance() {
        ...
        let input = input::generate(128, 2, Fr::MODULUS, true);
        ...
        let _params_1_to_11: PoseidonParameters<Fr> = generate(128, 2, Fr::MODULUS, true);

        // $t=3$ corresponds to a 2:1 hash
        let input = input::generate(128, 3, Fr::MODULUS, true);
        let rounds = rounds::generate(&input, &alpha);
        assert_eq!(rounds.full(), 8);
        assert_eq!(rounds.partial(), 31);
        let _params_2_to_1: PoseidonParameters<Fr> = generate(128, 3, Fr::MODULUS, true);

        // $t=4$ corresponds to a 3:1 hash
        let input = input::generate(128, 4, Fr::MODULUS, true);
        let rounds = rounds::generate(&input, &alpha);
        assert_eq!(rounds.full(), 8);
        assert_eq!(rounds.partial(), 31);
        let _params_3_to_1: PoseidonParameters<Fr> = generate(128, 4, Fr::MODULUS, true);

        // $t=5$ corresponds to a 4:1 hash
        let input = input::generate(128, 5, Fr::MODULUS, true);
        let rounds = rounds::generate(&input, &alpha);
        assert_eq!(rounds.full(), 8);
        assert_eq!(rounds.partial(), 31);
        let _params_4_to_1: PoseidonParameters<Fr> = generate(128, 5, Fr::MODULUS, true);

        // $t=6$ corresponds to a 5:1 hash
        let input = input::generate(128, 6, Fr::MODULUS, true);
        let rounds = rounds::generate(&input, &alpha);
        assert_eq!(rounds.full(), 8);
        assert_eq!(rounds.partial(), 31);
        let _params_5_to_1: PoseidonParameters<Fr> = generate(128, 6, Fr::MODULUS, true);
    }
}
